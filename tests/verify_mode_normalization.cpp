...
int main() {
  ...
  std::cout << "WR-90: a = " << a * 1000 << " mm, b = " << b * 1000 << " mm"
            << std::endl;
  ...
  double power_factor =
      omega * mu0 * beta * b / (4.0 * pi_sq); // × A² × a³ gives P
  ...
  std::cout << "Power with A_correct: " << P_correct << " (should be 1)"
            << std::endl;
  ...
  // Or: A² = 4 * kc⁴ * a / (ωμβπ²b)
  double A_sq_fixed =
      4.0 * std::pow(kc, 4) * a / (omega * mu0 * beta * pi_sq * b);
  ...
  std::cout << "Power with A_fixed: " << P_fixed << " (should be 1)"
            << std::endl;
  std::cout << std::endl;

  // Check ratio
  std::cout << "=== Final Check ===" << std::endl;
  std::cout << "A_fixed / A_correct = " << A_fixed / A_correct
  ...
  std::cout << "||w||²_code / ||w||²_correct = " << A_sq_code / A_sq_correct
            << std::endl;
  std::cout << "This means port admittance Y = ||w||²/Z0 is "
            << A_sq_code / A_sq_correct << "x too large!" << std::endl;
