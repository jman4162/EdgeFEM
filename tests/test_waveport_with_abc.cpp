...
int main() {
  ...
  std::cout << "S11 = " << S_noabc(0, 0)
            << " (|S11| = " << std::abs(S_noabc(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_noabc(1, 0)
            << " (|S21| = " << std::abs(S_noabc(1, 0)) << ")" << std::endl;
  ...
  std::cout << "S11 = " << S_abc(0, 0) << " (|S11| = " << std::abs(S_abc(0, 0))
            << ")" << std::endl;
  std::cout << "S21 = " << S_abc(1, 0) << " (|S21| = " << std::abs(S_abc(1, 0))
            << ")" << std::endl;
  ...
  std::cout << "S11 = " << S_10x(0, 0) << " (|S11| = " << std::abs(S_10x(0, 0))
            << ")" << std::endl;
  std::cout << "S21 = " << S_10x(1, 0) << " (|S21| = " << std::abs(S_10x(1, 0))
            << ")" << std::endl;
  ...
  std::cout << "S11 = " << S_100x(0, 0)
            << " (|S11| = " << std::abs(S_100x(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_100x(1, 0)
            << " (|S21| = " << std::abs(S_100x(1, 0)) << ")" << std::endl;

  // Try a different approach: normalize weights so ||w||²/Z0 matches matrix
  // diagonal Matrix diagonal ~ 1000, so we want ||w||²/Z0 ~ 1000, i.e., ||w||²
  // ~ 500000
  ...
  std::cout << "S11 = " << S_match(0, 0)
            << " (|S11| = " << std::abs(S_match(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_match(1, 0)
            << " (|S21| = " << std::abs(S_match(1, 0)) << ")" << std::endl;
  ...
  std::cout << "Phase(S21_expected) = " << std::arg(S21_expected) * 180 / M_PI
            << "°" << std::endl;

  // Check passivity for different scalings
  std::cout << "\n=== Passivity Check ===" << std::endl;
  std::cout << "No ABC:  |S11|² + |S21|² = "
            << std::norm(S_noabc(0, 0)) + std::norm(S_noabc(1, 0)) << std::endl;
  std::cout << "ABC:     |S11|² + |S21|² = "
            << std::norm(S_abc(0, 0)) + std::norm(S_abc(1, 0)) << std::endl;
  std::cout << "10x:     |S11|² + |S21|² = "
            << std::norm(S_10x(0, 0)) + std::norm(S_10x(1, 0)) << std::endl;
  std::cout << "100x:    |S11|² + |S21|² = "
            << std::norm(S_100x(0, 0)) + std::norm(S_100x(1, 0)) << std::endl;
  std::cout << "Matched: |S11|² + |S21|² = "
            << std::norm(S_match(0, 0)) + std::norm(S_match(1, 0)) << std::endl;
