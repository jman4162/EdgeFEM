...
int main() {
  ...
  WavePort wp1_eig = build_wave_port_from_eigenvector(
      mesh, port1_surf, v_te10, mode1, bc.dirichlet_edges);
  WavePort wp2_eig = build_wave_port_from_eigenvector(
      mesh, port2_surf, v_te10, mode2, bc.dirichlet_edges);

  std::cout << "\nPort 1 eigenvector weights ||w||² = "
            << wp1_eig.weights.squaredNorm() << std::endl;
  std::cout << "Port 2 eigenvector weights ||w||² = "
            << wp2_eig.weights.squaredNorm() << std::endl;
  std::cout << "Target ||w||² = sqrt(Z0) = " << std::sqrt(std::real(mode1.Z0))
            << std::endl;
  ...
  std::cout << "Analytical weights (normalized) ||w||² = "
            << wp1_ana.weights.squaredNorm() << std::endl;

  // Test 1: Analytical weights without port ABC
  std::cout << "\n=== Test 1: Analytical Weights (no port ABC) ==="
            << std::endl;
  MaxwellParams p1;
  p1.omega = omega;
  ...
  std::cout << "S11 = " << S_ana_abc(0, 0)
            << " (|S11| = " << std::abs(S_ana_abc(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_ana_abc(1, 0)
            << " (|S21| = " << std::abs(S_ana_abc(1, 0)) << ")" << std::endl;

  // Test 3: Eigenvector weights without port ABC
  std::cout << "\n=== Test 3: Eigenvector Weights (no port ABC) ==="
            << std::endl;
  std::vector<WavePort> ports_eig{wp1_eig, wp2_eig};
  auto S_eig = calculate_sparams(mesh, p1, bc, ports_eig);
  std::cout << "S11 = " << S_eig(0, 0) << " (|S11| = " << std::abs(S_eig(0, 0))
            << ")" << std::endl;
  std::cout << "S21 = " << S_eig(1, 0) << " (|S21| = " << std::abs(S_eig(1, 0))
            << ")" << std::endl;
  ...
  std::cout << "S11 = " << S_eig_abc(0, 0)
            << " (|S11| = " << std::abs(S_eig_abc(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_eig_abc(1, 0)
            << " (|S21| = " << std::abs(S_eig_abc(1, 0)) << ")" << std::endl;
  ...
  std::cout << "Analytical:     "
            << std::norm(S_ana(0, 0)) + std::norm(S_ana(1, 0)) << std::endl;
  std::cout << "Ana + ABC:      "
            << std::norm(S_ana_abc(0, 0)) + std::norm(S_ana_abc(1, 0))
            << std::endl;
  std::cout << "Eigenvector:    "
            << std::norm(S_eig(0, 0)) + std::norm(S_eig(1, 0)) << std::endl;
  std::cout << "Eig + ABC:      "
            << std::norm(S_eig_abc(0, 0)) + std::norm(S_eig_abc(1, 0))
            << std::endl;
