...
// For proper S-param extraction, we need V = sqrt(Z0) when A corresponds to
// unit power This requires ||w||² = sqrt(Z0) / A_unit, where A_unit = sqrt(2)
// (unit power normalization) So ||w||² = sqrt(Z0/2)
...
int main() {
  ...
  std::cout << "\n=== Normalized to ||w||² = sqrt(Z0) = " << target1
            << " ===" << std::endl;
  ...
  std::cout << "S11 = " << S_norm1(0, 0)
            << " (|S11| = " << std::abs(S_norm1(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_norm1(1, 0)
            << " (|S21| = " << std::abs(S_norm1(1, 0)) << ")" << std::endl;
  ...
  std::cout << "S11 = " << S_norm2(0, 0)
            << " (|S11| = " << std::abs(S_norm2(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_norm2(1, 0)
            << " (|S21| = " << std::abs(S_norm2(1, 0)) << ")" << std::endl;

  // Normalize so ||w||² = Z0
  double target3 = Z0;
  double scale3 = std::sqrt(target3 / w1_norm_sq);
  std::cout << "\n=== Normalized to ||w||² = Z0 = " << target3
            << " ===" << std::endl;
  ...
  std::cout << "S11 = " << S_norm3(0, 0)
            << " (|S11| = " << std::abs(S_norm3(0, 0)) << ")" << std::endl;
  std::cout << "S21 = " << S_norm3(1, 0)
            << " (|S21| = " << std::abs(S_norm3(1, 0)) << ")" << std::endl;
  ...
  for (double target :
       {0.1, 1.0, 10.0, 22.3, 100.0, 499.0, 1000.0, 5000.0, 38440.0}) {
    ...
    std::cout << std::setw(12) << target << "  " << std::setw(8)
              << std::abs(S_t(0, 0)) << "  " << std::setw(8)
              << std::abs(S_t(1, 0)) << "  " << (pass <= 1.01 ? "PASS" : "FAIL")
              << std::endl;
  }
