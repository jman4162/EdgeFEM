...
// Key question: WHY does the 3D eigenvector have only 3% correlation with 2D
// mode?
...
int main() {
  ...
  std::cout << "Solving 3D eigenvalue problem (" << n_free << " DOFs)..."
            << std::endl;
  ...
  std::vector<std::tuple<int, double, double, double, Eigen::Vector3d>>
      edge_data;
  ...
  for (size_t i = 0; i < wp1.edges.size(); ++i) {
    ...
    edge_data.push_back(
        {e, w_mag, v_mag, (w_mag > 1e-10 ? v_mag / w_mag : 0), mid});
  }
  ...
  std::cout << "TE10 has Ey only, so y-directed edges should dominate"
            << std::endl;
  ...
  std::cout << "X-directed edges: " << n_x_edges
            << ", sum |w| = " << sum_x_edges_w
            << ", sum |v_3d| = " << sum_x_edges_v << std::endl;
  std::cout << "Y-directed edges: " << n_y_edges
            << ", sum |w| = " << sum_y_edges_w
            << ", sum |v_3d| = " << sum_y_edges_v << std::endl;
  std::cout << "Ratio (Y/X) for w: " << sum_y_edges_w / sum_x_edges_w
            << std::endl;
  std::cout << "Ratio (Y/X) for v_3d: " << sum_y_edges_v / sum_x_edges_v
            << std::endl;
  ...
  if (max_z_count > 3) {
    std::cout << "Eigenvector at (x,y)≈(" << best_xy.first / 100.0 << ","
              << best_xy.second / 100.0 << ") mm:" << std::endl;
  ...
  // For TE10: Ey ∝ sin(πx/a), which varies from 0 at x=0 to 1 at x=a/2 to 0 at
  // x=a So edges near x=a/2 ≈ 11.4mm should have largest weight
