...
`simulate()` runs a full-wave FEM solve via lumped-port excitation (backed by
StackedPatchDesign) after `generate_mesh()`. `input_impedance()` and
`radiation_pattern()` always use analytical approximations (transmission-line
model, cavity model) and emit a warning saying so.
"""
    ...
    def _warn_analytical(self):
        ...
        if not self._analytical_warning_shown:
            warnings.warn(
                ...
                "For full-wave results call simulate() after generate_mesh().",
                stacklevel=3,
            )
